...
def main():
    #parse args then run it
    with data.change_git_dir('.'):
        args = parse_args()
        args.func(args)

def parse_args():
    parser = argparse.ArgumentParser() #create new arg parser

    #store the command in a variable and add it as subparser
    commands = parser.add_subparsers(dest = 'command')
    commands.required = True

    oid = base.get_oid

    #parse the init function
    init_parsers = commands.add_parser('init')
...
def init(args):
    base.init()
    print(f'Initialized empty ugit repository {os.getcwd()}/{data.GIT_DIR}')

def hash_object(args):
    #write object id as a hash
    with open(args.file, 'rb') as f:
        print(data.hash_object(f.read()))

def cat_file(args):
    #read a hash back to object id
    sys.stdout.flush()
...
def reset(args):
    base.reset(base.reset(args.commit))
...
def status(args):
    ...
    if(branch):
        ...
        print(f'HEAD attached to {HEAD[:10]}')
...
def k(args):
    ...
    with subprocess.Popen([dot_path, '-Tpng', '-o', 'graph.png'], stdin = subprocess.PIPE) as proc:
